...
public class VoiceAppLogger {
    ...
    private static HashMap<Context, File> contextVsFile = new HashMap<Context,File>();
    ...
    static void setContext(Context ctx) {
        currentContext = ctx;
        if(!contextVsFile.containsKey(ctx)) {
            ...
            File logFile = new File(currentContext.getFilesDir(), fileName);
            Log.i(TAG,"log file path : "+currentContext.getFilesDir().getAbsolutePath()+"/"+fileName);
            if (!logFile.exists())
            {
                try
                {
                ...
                {
                    Log.e(TAG,"exception when setting context : "+e.getLocalizedMessage());
    ...
    static void appendLog(String text)
    {
        if(null == currentContext) {
            Log.w(TAG,"current Context is not set yet");
            return;
        }
        if(buf == null) {
            return;
        ...
        try
        {
            ...
            buf.append(text);
            buf.newLine();
            // flushing to avoid loosing the buffered log in case of crash
            buf.flush();
        }
        catch (IOException|NullPointerException e)
        {
            Log.e(TAG,"exception when appending log : "+e.getLocalizedMessage());
            FirebaseCrashlytics.getInstance().recordException(e);
        }
    ...
    static void uploadAppLogsToServer(Date startDate, Date endDate, String signedUrl, String fileName) {
        ...
        File [] fileList = currentContext.getFilesDir().listFiles(filter);

        VoiceAppLogger.error(TAG,"Size of fileList: "+fileList.length);
        if(fileList.length > 0) {
            createZipFile(fileList,currentContext.getFilesDir().toString() + "/" + fileName);
    ...
    static void getSignedUrlForLogUpload() {
        ...
        SharedPreferencesHelper sharedPreferencesHelper = SharedPreferencesHelper.getInstance(currentContext);
    ...
    static void zipOlderLogs() {
        ...
        for(int i = 0; i < fileList.length; i++) {
            ...
            zipFileName = currentContext.getFilesDir() + "/" + zipFileName;
